...
export class SearchPage implements OnInit, OnDestroy {
  ...
  async loadData(searchQuery: string, filter: 'all' | 'team' | 'player' | 'league') {
    try {
      ...
      await this.loadPage(searchQuery, filter);
  ...
  async loadPage(searchQuery: string, filter: 'all' | 'team' | 'player' | 'league', event?: any) {
    ...
    try {
      ...
      console.error('Chyba při načítání dalších dat.', error);
    } finally {
      this.loadingMore[filter] = false;
    }
  }
